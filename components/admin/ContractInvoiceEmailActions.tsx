/**
 * Contract Invoice Email Actions Component
 * Provides buttons to preview, send, and test contract-invoice emails
 */
...
interface ContractInvoiceEmailActionsProps {
  contractId: string;
  invoiceId: string;
...
export default function ContractInvoiceEmailActions({
  contractId,
  invoiceId,
  contractNumber,
  invoiceNumber,
  eventName,
  disabled = false
}: ContractInvoiceEmailActionsProps) {
  ...
  const handleSend = async () => {
    if (!confirm(`Are you sure you want to send the contract and invoice email to the client?`)) {
      return;
    }
    ...
    try {
      ...
      toast({
        title: 'Email Sent',
        description: 'Contract and invoice email sent successfully to client'
      });
    } catch (error: any) {
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <DialogTitle className="flex items-center justify-between">
              <span>Email Preview: {previewSubject || 'Contract & Invoice Ready'}</span>
              <Button
                variant="ghost"
                size="sm"
