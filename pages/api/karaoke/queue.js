    ...
    if (event_code && event_code !== 'all') {
      ...
      if (signupsError) {
        ...
        const errorDetails = {
          ...
          message: signupsError.message || String(signupsError),
        ...
        // Handle column doesn't exist error (42703) - might be missing migration
        if (signupsError.code === '42703') {
          console.warn('Column does not exist error - migration may be missing');
          // Try query without the problematic columns
          ...
          if (event_code && event_code !== 'all') {
            fallbackQuery.eq('event_qr_code', event_code);
          }
          
          const { data: fallbackSignups, error: fallbackError } = await fallbackQuery;
          
          if (!fallbackError && fallbackSignups) {
            console.log('Fallback query succeeded, returning queue without video metadata');
            ...
            const formattedQueue = queue.map((signup, index) => ({
              ...
              estimated_wait: formatEstimatedWait(calculateEstimatedWait(signup, safeSignups)),
            ...
            return res.status(200).json({
              ...
              warning: 'Some video metadata columns are missing - migration may be required'
            });
          }
        }
        ...
        return res.status(500).json({
          ...
          details: errorDetails.message || String(signupsError),
