  ...
  const linkSuggestedVideoToSignup = async (signupId: string, videoData: any) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Batch all state updates together to prevent DOM conflicts
        setSignups(prev => prev.map(signup =>
          signup.id === signupId
        ...
        // Clear the suggestion after a brief delay to prevent DOM conflicts
        ...
        console.log('Video linking completed successfully for signup:', signupId);
        // Small delay to let the modal animation complete before showing toast
        setTimeout(() => {
          if (isMountedRef.current) {
            toast({
  ...
  const updateStatus = async (signupId: string, newStatus: KaraokeSignup['status'], adminNotes?: string) => {
    ...
    try {
      ...
      if (newStatus === 'singing') {
        // Create the updated signup object with the new status
        const updatedSignup = { ...originalSignup, status: newStatus };
        if (updatedSignup && updatedSignup.video_data && karaokeLayoutRef.current) {
          ...
          // Use setTimeout to ensure state is updated before calling startPlayingSignup
          setTimeout(() => {
            ...
            <DialogContent className="max-w-4xl max-h-[95vh] overflow-y-auto">
              ...
              {selectedSignup && (
                ...
                >
                  ...
                  {selectedSignup.song_title && (
                    <div key={`video-section-${selectedSignup.id}`} className="mt-6">
                      ...
                      {selectedSignup.video_data ? (
                        <div key="linked-video" className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                        ...
                        <div className="space-y-3">
                          ...
                          {loadingVideoSuggestions[selectedSignup.id] ? (
                            <div key="loading-suggestion" className="p-4 bg-gray-50 dark:bg-gray-900/20 rounded-lg border border-gray-200 dark:border-gray-800">
