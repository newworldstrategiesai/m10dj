-- Check if user_video_library table exists and create if not
SELECT
  CASE
    WHEN EXISTS (
    ...
    ELSE 'Table does not exist - creating...'
  END as status;
...
DROP POLICY IF EXISTS "Users can insert videos to their library" ON user_video_library;
