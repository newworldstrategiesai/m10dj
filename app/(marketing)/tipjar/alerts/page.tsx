...
export default function StreamAlertsPageQuery() {
  ...
  useEffect(() => {
    ...
    async function loadConfig() {
      try {
        ...
        if (!userParam) {
          setError('User parameter required. Use ?user=username or ?user=user_id');
          setLoading(false);
          return;
        }

        // Try to find user by username, alert_token, or user_id
        let userId: string | null = null;

        const { data: configData } = await supabase
          .from('stream_alert_configs')
          .select('user_id, username, alert_token')
          .or(`username.eq.${userParam},alert_token.eq.${userParam},user_id.eq.${userParam}`)
          .single();
        ...
        if (configError || !fullConfig) {
          setError('Configuration not found');
          setLoading(false);
          return;
        }

        const typedFullConfig = fullConfig as StreamAlertConfig;

        if (mounted) {
          setConfig(typedFullConfig);
        }
        ...
        const { data: donorsData } = await supabase
          ...
          .limit(typedFullConfig.ticker_count || 5);
          ...
          .on(
            ...
            async (payload: any) => {
              ...
              if (mounted) {
                ...
                if (typedFullConfig.goal_enabled && newEvent.event_type === 'tip' && newEvent.event_data?.amount) {
                  const newGoalCurrent = (typedFullConfig.goal_current || 0) + parseFloat(newEvent.event_data.amount);
                  await (supabase
